...
gulp.task('watch', function () {
    gulp.watch(paths.sass, ['sass']);
    // NOTE: gulp watch does not automatically load new script and css files
    // without the line
    gulp.watch([paths.scripts, paths.css], ['index']);
    // which is not used to avoid following log output:
    //   1     186528   log      LiveReload protocol error (invalid command
    //   'reload', only valid commands are: hello)) after receiving data:
    //   "{"command":"reload","path":"www/css/ionic.app.min.css","liveCss":true,
    //   "liveJs":true,"liveImg":true}"..
});
