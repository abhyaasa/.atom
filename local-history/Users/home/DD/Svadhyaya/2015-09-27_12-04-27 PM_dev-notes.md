...
`gulp config` transfers name, email, href, and version from config.xml to www/data/config.json. The config.json file also contains the flavor attribute, which is set by `gulp flavor`. Early in app initialization, the config object denoted by this file is stored in $rootScope. 
