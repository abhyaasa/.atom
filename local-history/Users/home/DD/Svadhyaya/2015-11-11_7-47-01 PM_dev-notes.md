...
- the `debug` logging service is enabled. This service is also enabled if `$rootScope.config` has not been defined yet. In such cases `$log.debug` should be used instead.
