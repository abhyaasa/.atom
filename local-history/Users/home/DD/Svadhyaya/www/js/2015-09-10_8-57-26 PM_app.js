...
.config(function ($stateProvider, $urlRouterProvider, $logProvider) {
    ...
    $stateProvider
        .state('tabs', {
            ...
            controller: function ($scope, configPromise, $log) {
                $log.debug('in tabs controller');
                $scope.config = configPromise.data;
                $log.debug('config:', JSON.stringify($scope.config));
            }
        })
        .state('tabs.decks', {
            url: '/decks',
...
.run(function ($ionicPlatform, $rootScope, $log, restoreSettings) {
    $log.debug('in run');
