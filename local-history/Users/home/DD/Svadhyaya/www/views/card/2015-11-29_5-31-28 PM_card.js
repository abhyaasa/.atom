...
.controller('CardController', function ($scope, $log, _, Deck, Card, $ionicGesture) {
    ...
    $scope.$on('$ionicView.enter', function() {
        ...
        $scope.haveHint = Card.question.hints !== undefined;
        $scope.hint = null;
    });

    $scope.hint = function () {
        $scope.hint = Card.question.hints[Card.hintIndex];
        Card.hintIndex++;
        $scope.haveHint = Card.hintIndex < Card.question.hints.length;
    };
    ...
    this.setup = function (activeCardIndex) {
        ...
        if (Card.question.type === 'multiple-choice') {
            ...
            if (answerIndex === Deck.questions.length) {
                ...
                Card.question.answer = Deck.questions[answerIndex];
            }
        }
