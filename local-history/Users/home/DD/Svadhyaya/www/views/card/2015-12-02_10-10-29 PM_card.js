...
.service('Card', function ($log, $state, Deck, settings, _) {
    ...
    this.setup = function (activeCardIndex) {
        ...
        if (Card.question.type === 'true-false') {
            ...
            var numResponses = 5;
            responses = _.sample(responses, numResponses - 1).push(q.answer);
            responses = _.sample(responses, numResponses);
            var r = Deck.questions.slice(q.matchingBegin, q.id - 1)
            .concat(Deck.questions.slice(q.id + 1, q.matchingEnd + 1);
            r = Deck.questions.slice(q.matchingEnd + 1));
        }
