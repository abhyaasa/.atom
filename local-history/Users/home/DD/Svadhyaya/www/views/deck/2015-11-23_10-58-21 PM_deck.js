...
.service('Deck', function ($log, $state, Card, config, getData, _) {
    ...
    var outcomeNames = ['right', 'wrong', 'close', 'skipped'];
    this.outcome = function (outcome) {
        if (!_.contains(outcomeNames, outcome)) {
    ...
    this.getCount = function () {
        var count = {};
        _.forEach(outcomeNames, function (outcome) {
            // XXX count[outcome] = _.filter(Deck.data.outcomes, function () {
        });
        return count;
