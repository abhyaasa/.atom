...
.controller('LibraryController', function ($rootScope, $scope, $state, configPromise,
  debug, getData, deckSetup, _, $log) {
    // PUBLISH remove all $log.debug calls, and maybe debug calls
    $log.debug('in LibraryController', configPromise);
    $log.debug(JSON.stringify(configPromise.data));
    ...
    getData(indexFile).then(function (promise) {
        ...
        if ($scope.deckList.length === 1 || $rootScope.debug) {
