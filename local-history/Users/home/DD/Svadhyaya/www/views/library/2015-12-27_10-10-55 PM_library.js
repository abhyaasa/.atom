...
.controller('LibraryController', function ($rootScope, $scope, $state, $log, _, mode,
  Library, Deck, indexPromise) {
    $scope.selectDeck = function(displayName) {
        Deck.setup(deckName);
    };
...
.service('Library', function ($log, $state, getData, LocalStorage, _) {
    ...
    var updateDeckNames = function () {
        var closedFileDecks = _.reject(fileDecks, function (fd) {
            return _.contains(openFileNames, fd.display);
        });
        Library.deckNames = {
            open: openDecks.sort(),
            closed: closedFileDecks.sort()
        };
    };
