...
`./resources` link points to `data/flavors/<current flavor>/resources/` to keep the `ionic resources` command happy.
...
- `g ALIAS` is a shortcut for invoking via `gulp cmd` the script associased with `ALIAS` in the `cmdAliases` dictionary defined early in `gulpfile.js`.
