...
.sa : sanskrit answers/distractor text
...
.st : sanskrit question text, displayed in devanagari with IAST transliteration answer
...
special_apptime_tags = '.st .cs .ci .ma .html'.split()
...
def main(args):
    ...
    for elt in _input[1:].split('\n;'):
        ...
        if ';' not in elt:  # tag range
            ...
            q['text'] = do_text(trlst[0], '.sq' in qtags)
